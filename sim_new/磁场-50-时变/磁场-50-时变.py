...
class SimulationParameters:
    ...
    # --- 定义时变磁场的参数 ---
    B0_static = 1.0e4  # 初始磁场的静态分量 (T)
    B_amp = 1.0e4  # 磁场振荡的幅度 (T)
    omega_ratio = 0.1  # 磁场振荡角频率与等离子体频率之比 (ω / w_pe)
...
class PairPlasmaReconnection(object):  # Class name updated for clarity
    def __init__(self, params: SimulationParameters, test: bool, verbose: bool):
        ...
        # --- 复制时变磁场参数 ---
        self.B0_static = self.p.B0_static
        self.B_amp = self.p.B_amp
        self.omega_ratio = self.p.omega_ratio
        ...
        # --- 计算物理振荡频率 ω ---
        self.omega = self.omega_ratio * self.w_pe
        ...
        self.Bx = "0.0"
        self.By = "0.0"
        self.Bz = f"{self.B0_static} + {self.B_amp} * sin({self.omega} * t)"
        if comm.rank == 0:
            print("\n--- Initializing a TIME-VARYING magnetic field ---")
            print(f"    Bz(t) = {self.B0_static:.2e} + {self.B_amp:.2e} * sin({self.omega:.2e} * t) [T]\n")
    ...
    def setup_run(self):
        ...
        # 1. 设置 t=0 时的初始静态背景场
        #    这个表达式只在模拟开始时计算一次。
        B_initial = picmi.AnalyticInitialField(
            Bx_expression="0.0",
            By_expression="0.0",
            Bz_expression=f"{self.B0_static}"  # 在 t=0, sin(0)=0, 所以初始场就是 B0_static
        )
        simulation.add_applied_field(B_initial)

        # 2. 施加 t>0 时的纯粹时变部分
        #    这个表达式会在每个时间步重新计算。
        B_applied = picmi.AnalyticAppliedField(
            Bx_expression="0.0",
            By_expression="0.0",
            Bz_expression=f"{self.B_amp} * sin({self.omega} * t)"
        )
        simulation.add_applied_field(B_applied)
