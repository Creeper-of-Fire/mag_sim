...
#
#     # --- 1. 创建 Figure 和 GridSpec 布局 ---
#     # 高度可以根据模拟数量动态调整
#     fig_height = 6 + num_runs * 1.5
#     fig = plt.figure(figsize=(14, fig_height))
#
#     # 将Figure分为上下两部分：上部分是图，下部分是表
#     gs_main = gridspec.GridSpec(2, 1, figure=fig, height_ratios=[num_runs, 1.2], hspace=0.4)
#
#     # 再将上部分细分为左右两列
#     gs_plots = gridspec.GridSpecFromSubplotSpec(num_runs, 3, subplot_spec=gs_main[0], wspace=0.3)
#
#     # 定义主图和子图
#     ax_main = fig.add_subplot(gs_plots[:, 0:2])  # 主图占左边2/3
#     ax_subplots = [fig.add_subplot(gs_plots[i, 2], sharex=ax_main, sharey=ax_main) for i in range(num_runs)]
#
#     # 定义表格区域
#     ax_table = fig.add_subplot(gs_main[1])
#
#     fig.suptitle(f"模拟对比: {', '.join([run.name for run in runs])}", fontsize=18, y=0.98)
#
#     # --- 2. 准备数据和颜色 (代码不变) ---
#     if len(runs) <= 10:
...
#
#     if not all_energies:
#         ax_main.text(0.5, 0.5, '所有模拟均无能谱数据', ha='center', va='center', color='red')
#     else:
...
#                     valid_mask = dN_dE > 0
#                     # 在主图上画
#                     ax_main.plot(bin_centers_MeV[valid_mask], dN_dE[valid_mask], linestyle='-', color=color, lw=2.0,
#                                  label=f'{run.name}')
#                     # 在右侧小图上也画
#                     ax_sub.plot(bin_centers_MeV[valid_mask], dN_dE[valid_mask], linestyle='-', color=color, lw=2.0,
#                                 label='最终')
#
#                 # 绘制初始谱 (只在右侧小图上)
#                 if run.initial_spectrum:
...
#                     dN_dE = counts / bin_widths_MeV
#                     valid_mask = dN_dE > 0
#                     ax_sub.plot(bin_centers_MeV[valid_mask], dN_dE[valid_mask], linestyle='-', color='gray', lw=1.5,
#                                 label='初始')
#
#
#
#                 # 设置右侧小图的属性
#                 ax_sub.set_title(f'{run.name}', fontsize=10)
#                 ax_sub.legend(fontsize=8, loc='lower left')
...
#             ref_run = runs[1]
#             if ref_run.final_spectrum:
#                 T_plasma_J = ref_run.sim.T_plasma * E
#                 # 理论谱的归一化系数是加权后的总粒子数
#                 total_thermal_particles = np.sum(ref_run.final_spectrum.weights) * (1.0 - ref_run.sim.beam_fraction)
...
#                 ax_main.plot(bin_centers_MeV, dN_dE_juttner, '--', color='black', alpha=0.7,
#                              label=f'理论热谱 (T={ref_run.sim.T_plasma / 1e3:.1f} keV)')
#
#
#     # --- 4. 设置主图属性 ---
#     ax_main.set_title('最终能谱对比', fontsize=14)
#     ax_main.set_xscale('log')
#     ax_main.set_yscale('log')
#     ax_main.set_xlabel('动能 (MeV)', fontsize=12)
#     ax_main.set_ylabel('粒子数谱密度 (dN/dE [MeV⁻¹])', fontsize=12)
#     ax_main.legend()
...
def generate_comparison_plot(runs: List[SimulationRun]):
    """为选定的模拟运行生成一张单列、大尺寸的包含能谱网格和参数表的摘要图。"""
    console.print("\n[bold magenta]正在生成对比图...[/bold magenta]")
    ...
    # MODIFICATION: 增加每张子图的高度，并调整整体画布尺寸
    plot_height_per_run = 12  # 每张子图的高度（英寸）
    headers, param_rows, cell_text = _prepare_table_data(runs)
    table_height = max(3.5, len(param_rows) * 0.28)  # 表格高度

    # 计算总高度和宽度
    fig_height = plot_height_per_run * nrows + table_height
    fig_width = 12  # 固定一个较宽的尺寸

    fig = plt.figure(figsize=(fig_width, fig_height))

    # 主网格：上部分是图，下部分是表
    gs_main = gridspec.GridSpec(2, 1, figure=fig,
                                height_ratios=[plot_height_per_run * nrows, table_height],
                                hspace=0.6)

    # 子图网格
    gs_plots = gridspec.GridSpecFromSubplotSpec(nrows, ncols, subplot_spec=gs_main[0], hspace=0.3)
    axes = [fig.add_subplot(gs_plots[i, 0]) for i in range(num_runs)]

    fig.suptitle(f"模拟对比分析", fontsize=20, y=0.99)
    ...
    if not all_energies:
        fig.text(0.5, 0.7, '所有模拟均无能谱数据', ha='center', va='center', color='red', fontsize=16)
    else:
        combined_energies = np.concatenate(all_energies)
        positive_energies = combined_energies[combined_energies > 0]
        if positive_energies.size > 1:
            num_bins = 200  # 增加分箱数以获得更平滑的曲线
            min_E = max(positive_energies.min() * 0.5, 1e-4)
            max_E = positive_energies.max() * 1.2
            common_bins_MeV = np.logspace(np.log10(min_E), np.log10(max_E), num_bins + 1)
            bin_centers_MeV = np.sqrt(common_bins_MeV[:-1] * common_bins_MeV[1:])
            ...
            for i, (run, ax) in enumerate(zip(runs, axes)):
                if run.initial_spectrum:
                    ...
                    dN_dE = counts / bin_widths_MeV
                    valid_mask = dN_dE > 0
                    ax.plot(bin_centers_MeV[valid_mask], dN_dE[valid_mask], linestyle='--', color='gray', lw=2,
                            label='初始')
                ...
                if run.final_spectrum:
                    ...
                    dN_dE = counts / bin_widths_MeV
                    valid_mask = dN_dE > 0
                    ax.plot(bin_centers_MeV[valid_mask], dN_dE[valid_mask], linestyle='-', color=colors[i], lw=2.5,
                            label='最终')
                ...
                # # MODIFICATION: 只在最下面的图显示x轴标签
                # if i < num_runs - 1:
                #     ax.set_xticklabels([])

    # --- 4. 设置共享坐标轴标签 ---
    fig.supxlabel('动能 (MeV)', fontsize=16)
    fig.supylabel('粒子数谱密度 (dN/dE [MeV⁻¹])', fontsize=16, x=0.04)

    # --- 5. 绘制参数表 ---
    ax_table = fig.add_subplot(gs_main[1])
    ax_table.axis('off')
    ax_table.set_title('模拟参数对比', fontsize=16, y=1.0, pad=20)
    ...
    for (row, col), cell in table.get_celld().items():
        ...
        if col == -1:
            ...
            if "---" in cell.get_text().get_text():
                ...
                cell.set_facecolor('#E0E0E0')
        # 数据单元格交替颜色
        if row >= 0 and col >= 0 and row % 2 == 1:
