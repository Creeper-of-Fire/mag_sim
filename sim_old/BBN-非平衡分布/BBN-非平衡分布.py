...
class PairPlasmaReconnection(object):  # Class name updated for clarity
    ...
    def setup_run(self):
        ...
        def get_mj_average_energy(T_eV):
            """计算麦克斯韦-Jüttner分布在给定温度下的精确平均动能 (单位: 焦耳)"""
            ...
            # 公式: <E_k> = m*c^2 * (3*theta + K_1(1/theta)/K_2(1/theta) - 1)
            # K_n 是 n 阶第二类修正贝塞尔函数
            val = 1.0 / theta
            avg_gamma = 3.0 * theta + kv(1, val) / kv(2, val)
            avg_energy_J = m_e_c2 * (avg_gamma - 1.0)
            return avg_energy_J

        def solve_for_umax(target_avg_energy_J, k_exponent=0.5):
            """
            求解 u_max，使得幂律分布 u(z) = u_max * z_norm^k 的平均能量
            等于 target_avg_energy_J。
            """
            m_e_c2 = constants.m_e * constants.c ** 2

            # 定义动能 E_k 作为 z_norm 的函数
            def energy_integrand(z_norm, u_max):
                u_val = u_max * (z_norm ** k_exponent)
                gamma = np.sqrt(1.0 + u_val ** 2)
                return m_e_c2 * (gamma - 1.0)

            # 定义一个函数，其根是我们想找的 u_max
            def residual(u_max):
                # 计算当前 u_max 对应的平均能量 (通过对 z_norm 从 0 到 1 积分)
                avg_energy = quad(energy_integrand, 0, 1, args=(u_max,))[0]
                return avg_energy - target_avg_energy_J

            # 使用求根算法求解 u_max
            # 提供一个合理的搜索区间，例如 u 对应 0.01c 到 10c
            sol = root_scalar(residual, bracket=[0.01, 10.0], method='brentq')
            if not sol.converged:
                raise RuntimeError("未能为 u_max 找到收敛解！")

            return sol.root

        # --- 为构建一个总能量合理的幂律谱计算动量尺度 ---
        # 1. 计算 T=84keV 时的目标平均动能
        target_avg_energy_J = get_mj_average_energy(self.T_plasma)
        print(f"\n--- 能量匹配计算 ---")
        print(f"目标平均动能 (84 keV 热分布): {target_avg_energy_J / e / 1e3:.3f} keV")

        # 2. 定义幂律指数 k
        powerlaw_k_exponent = 0.5

        # 3. 求解能实现此平均能量所需的 u_max
        u_max_solved = solve_for_umax(target_avg_energy_J, k_exponent=powerlaw_k_exponent)
        u_min = 0.0  # 我们从0开始
        print(f"求解得到的 u_max: {u_max_solved:.4f} (对应 p_max/mc)")
        print(f"----------------------\n")

        # 4. 创建归一化 z 坐标表达式
        z_norm_expr = f"((z + {self.Lz / 2.0}) / {self.Lz})"

        # 5. 构建最终的幂律映射表达式
        momentum_expr_uz = f"{u_min} + ({u_max_solved - u_min}) * pow({z_norm_expr}, {powerlaw_k_exponent})"
        momentum_expr_ux = "0.0"
        momentum_expr_uy = "0.0"

        # --- 通过 Bucket 注入幂律动量梯度函数 ---
        electrons_bucket: Bucket = self.electrons.species
        positrons_bucket: Bucket = self.positrons.species

        # 为电子注入
        electrons_bucket.add_new_attr("momentum_distribution_type", "parse_momentum_function")
        electrons_bucket.add_new_attr("momentum_function_ux(x,y,z)", momentum_expr_ux)
        electrons_bucket.add_new_attr("momentum_function_uy(x,y,z)", momentum_expr_uy)
        electrons_bucket.add_new_attr("momentum_function_uz(x,y,z)", momentum_expr_uz)

        # 为正电子注入
        positrons_bucket.add_new_attr("momentum_distribution_type", "parse_momentum_function")
        positrons_bucket.add_new_attr("momentum_function_ux(x,y,z)", momentum_expr_ux)
        positrons_bucket.add_new_attr("momentum_function_uy(x,y,z)", momentum_expr_uy)
        positrons_bucket.add_new_attr("momentum_function_uz(x,y,z)", momentum_expr_uz)
