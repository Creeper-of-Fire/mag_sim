...
class PairPlasmaReconnection(object):  # Class name updated for clarity
    ...
    def setup_run(self):
        ...
        print("--- Adding Binary Coulomb Collisions for Thermalization ---")

        # 1. 电子与正电子之间的碰撞
        ep_collisions = picmi.CoulombCollisions(
            name='ep_collisions',
            species=[self.electrons, self.positrons],
            CoulombLog=None,  # 让 WarpX 根据局部参数自动计算，这是最精确的方式
            ndt=5  # 每 5 个时间步计算一次碰撞，在精度和性能间取得平衡
        )

        # 2. 电子与电子之间的碰撞 (自身物种碰撞)
        ee_collisions = picmi.CoulombCollisions(
            name='ee_collisions',
            species=[self.electrons, self.electrons],  # 注意这里是自己和自己
            CoulombLog=None,
            ndt=5
        )

        # 3. 正电子与正电子之间的碰撞 (自身物种碰撞)
        pp_collisions = picmi.CoulombCollisions(
            name='pp_collisions',
            species=[self.positrons, self.positrons],  # 注意这里是自己和自己
            CoulombLog=None,
            ndt=5
        )

        all_collisions = [ep_collisions, ee_collisions, pp_collisions]
        global simulation
        simulation = picmi.Simulation(
            warpx_serialize_initial_conditions=True,
            verbose=0,
            warpx_collisions=all_collisions
        )
        ...
        # --- 设置一个简单的、远离平衡态的初始条件：反向流动的冷束 ---
        # 这是一个典型的非平衡态设置，会激发强烈的等离子体不稳定性，
        # 从而使系统向更稳定的状态“弛豫”。

        # 定义一个初始的归一化动量 u = p / (m*c)
        # u = 1.0 对应洛伦兹因子 gamma = sqrt(1 + u^2) = sqrt(2) ~= 1.414
        # 速度 v = c * u / gamma ~= 0.707c
        u_drift = 1.0

        print(f"\n--- 非平衡态初始条件 ---")
        print(f"设置反向流动的冷束 (Counter-streaming Beams)")
        print(f"电子 u_z = +{u_drift:.2f}")
        print(f"正电子 u_z = -{u_drift:.2f}")
        print(f"------------------------\n")
        ...
        # 为电子设置 +z 方向的动量
        electrons_bucket.add_new_attr("momentum_distribution_type", "constant")
        electrons_bucket.add_new_attr("ux", 0.0)
        electrons_bucket.add_new_attr("uy", 0.0)
        electrons_bucket.add_new_attr("uz", u_drift)

        # 为正电子设置 -z 方向的动量
        positrons_bucket.add_new_attr("momentum_distribution_type", "constant")
        positrons_bucket.add_new_attr("ux", 0.0)
        positrons_bucket.add_new_attr("uy", 0.0)
        positrons_bucket.add_new_attr("uz", -u_drift)
