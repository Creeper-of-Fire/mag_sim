...
def juttner_model_for_fit(E_J, A, T_J):
    """
    用于 scipy.optimize.curve_fit 的麦克斯韦-Jüttner模型。
    参数:
    E_J: 能量 (焦耳)
    A: 归一化系数 (约等于总粒子数 N_total)
    T_J: 温度 (焦耳)
    返回:
    dN/dE (单位: 粒子数/焦耳)
    """
    pdf_per_J = get_maxwell_juttner_distribution(E_J, T_J)
    return A * pdf_per_J
    ...
    if all_energies_list:
        ...
        if positive_energies.size > 1:
            ...
            if final_energies is not None:
                ...
                bin_centers_J = bin_centers_MeV * J_per_MeV

                # 提供合理的初始猜测值
                avg_E_final_J = np.average(final_energies, weights=final_weights) * J_per_MeV
                T_guess_J = (2. / 3.) * avg_E_final_J  # 使用非相对论近似作为初始猜测
                p0 = [N_total_final, T_guess_J]

                try:
                    # 只对非零数据点进行拟合
                    mask = dN_dE_final > 0

                    # 单位转换：dN/dE_final 的单位是 粒子数/MeV，拟合函数需要 粒子数/J
                    dN_dE_final_per_J = dN_dE_final / J_per_MeV

                    popt, pcov = curve_fit(
                        juttner_model_for_fit,
                        bin_centers_J[mask],
                        dN_dE_final_per_J[mask],
                        p0=p0,
                        bounds=([0, 0], [np.inf, np.inf])  # 确保参数为正
                    )

                    A_fit, T_fit_J = popt

                    T_fit_MeV = T_fit_J / J_per_MeV

                    T_fit_MeV = 0.120
                    T_fit_J = T_fit_MeV * J_per_MeV

                    # 使用拟合参数生成理论曲线
                    fit_curve_per_J = juttner_model_for_fit(bin_centers_J, A_fit, T_fit_J)
                    fit_curve_per_MeV = fit_curve_per_J * J_per_MeV  # 转换回 粒子数/MeV

                    ax1.plot(bin_centers_MeV, fit_curve_per_MeV, '--', color='black', lw=2,
                             label=f'Jüttner 拟合 (T_eff = {T_fit_MeV * 1e3:.2f} keV)')

                    print("\n--- 弛豫测试诊断 ---")
                    print(f"初始平均动能: {np.average(initial_energies, weights=initial_weights):.3f} MeV")
                    print(f"最终平均动能: {np.average(final_energies, weights=final_weights):.3f} MeV")
                    print(f"拟合得到的有效温度: {T_fit_MeV:.3f} MeV ({T_fit_MeV * 1e3:.1f} keV)")
                    print(f"拟合得到的总粒子数: {A_fit:.2e} (模拟值为: {N_total_final:.2e})")
                    print("----------------------\n")

                except RuntimeError as e:
                    print(f"     -> 警告: 曲线拟合失败: {e}")
                    ax1.text(0.5, 0.5, '曲线拟合失败', horizontalalignment='center', transform=ax1.transAxes)
        else:
