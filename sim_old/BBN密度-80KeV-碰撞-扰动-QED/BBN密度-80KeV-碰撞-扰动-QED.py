...
from pywarpx import callbacks, fields, libwarpx, picmi, warpx
...
class SimulationParameters:
    ...
    beam_u_drift = 1.0   # 束流的归一化动量 u = p/(mc)
...
class PairPlasmaReconnection(object):  # Class name updated for clarity
    ...
    def setup_run(self):
        ...
        all_collisions = self.create_collision_pairs(all_charged_species, ndt=5)

        # --- MODIFICATION START 3: Define and Inject QED-like Collisions ---
        # 由于picmi没有LinearCompton和LinearBreitWheeler的高级接口,
        # 我们必须使用我们之前确定的底层Bucket方法来注入这些碰撞的C++参数。

        print("\n--- Injecting Linear Compton and Breit-Wheeler Collisions via Bucket ---")

        # 1. 定义所有新碰撞的名称
        collision_names = [
            "compton_phot_ele_th", "compton_phot_ele_bm",
            "compton_phot_pos_th", "compton_phot_pos_bm",
            "bw_phot_phot"
        ]

        # 2. 创建并注册 'collisions' Bucket
        collisions_bucket = Bucket('collisions')
        pywarpx.warpx._bucket_dict['collisions'] = collisions_bucket
        # 将碰撞名称列表添加到这个Bucket中
        collisions_bucket.add_new_attr("collision_names", " ".join(collision_names))

        collision_ndt = 5

        # 3. 为每个具体的碰撞过程创建并注册Bucket
        def setup_collision_bucket(name, type, species, product_species):
            bucket = Bucket(name)
            pywarpx.warpx._bucket_dict[name] = bucket
            bucket.add_new_attr("type", type)
            bucket.add_new_attr("species", species)
            bucket.add_new_attr("product_species", product_species)
            bucket.add_new_attr("ndt", collision_ndt)
            print(f"  - Registered Bucket '{name}' for '{type}' collision.")

        # -- 康普顿散射 --
        setup_collision_bucket("compton_phot_ele_th", "linear_compton", ["photons", "electrons_thermal"], ["photons", "electrons_thermal"])
        setup_collision_bucket("compton_phot_ele_bm", "linear_compton", ["photons", "electrons_beam"], ["photons", "electrons_beam"])
        setup_collision_bucket("compton_phot_pos_th", "linear_compton", ["photons", "positrons_thermal"], ["photons", "positrons_thermal"])
        setup_collision_bucket("compton_phot_pos_bm", "linear_compton", ["photons", "positrons_beam"], ["photons", "positrons_beam"])

        # -- 线性Breit-Wheeler --
        setup_collision_bucket("bw_phot_phot", "linear_breit_wheeler", ["photons", "photons"], ["electrons_thermal", "positrons_thermal"])
        ...
        print(f"Photon population: Isotropic Gaussian with u_th ~ Theta = {theta_plasma:.3f}")
        print(f"--------------------------------------------\n")
        ...
        # 为光子设置一个与等离子体温度相匹配的初始动量分布。
        # 我们使用各向同性的高斯分布，其归一化动量标准差 u_th ~ kT / (m_e c^2) = theta
        # 注意: 此处的动量是按电子静止质量 m_e 归一化的, 这是WarpX的标准做法。
        photons_bucket: Bucket = self.photons.species

        # 1. 设置注入方式和每个单元的粒子数
        num_photons_per_cell = int(self.NPPC * self.p.n_photon_to_plasma_ratio)
        photons_bucket.add_new_attr("injection_style", "NRandomPerCell")
        photons_bucket.add_new_attr("num_particles_per_cell", num_photons_per_cell)

        # 2. 补上缺失的 profile 参数
        #    我们想要一个均匀的密度分布，所以 profile 应该是 'constant'。
        #    这个参数会告诉 WarpX 使用我们提供的 'density' 值作为整个区域的常数密度。
        photons_bucket.add_new_attr("profile", "constant")
        photons_bucket.add_new_attr("density", self.n_photon)

        # 3. 设置光子的初始动量分布
        photons_bucket.add_new_attr("momentum_distribution_type", "gaussian")
        photons_bucket.add_new_attr("ux_th", theta_plasma)
        photons_bucket.add_new_attr("uy_th", theta_plasma)
        photons_bucket.add_new_attr("uz_th", theta_plasma)
