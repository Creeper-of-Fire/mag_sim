...
def compute_run_tail_metrics(run: 'SimulationRunSingle', is_final: bool) -> Dict[str, float]:
    """
    计算单个 Run 的物理度量，并将这个小巧的字典（几十个字节）缓存到磁盘！
    下次再读取时，连 HDF5 都不用碰，直接秒读这个字典。
    """
    # 1. 取出对应的能谱 (此时没有缓存，直接读 HDF5)
    ...
    # === 把你原本 _analyze_spectrum_excess 里的核心物理逻辑搬到这里 ===
    # 1. 精确基础统计
...
class ParametricTailDebugModule(BaseComparisonModule):
    ...
    def _get_metrics_with_error(self, run_or_group, is_final: bool) -> Dict[str, float]:
        """
        核心分发器：
        如果是 Single Run，直接读缓存并返回。
        如果是 Group Run，遍历内部的 Single Run，读缓存，然后计算平均值和标准差！
        """
        from analysis.core.simulationGroup import SimulationRunGroup  # 确保导入
