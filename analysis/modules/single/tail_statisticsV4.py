...
class TailStatisticsModule(BaseAnalysisModule):
    @property
    def name(self) -> str:
        return "高能尾巴统计显著性分析v4(加权修正)"

    @property
    def description(self) -> str:
        return "【v4】分析高能尾部的宏粒子数量统计。v4修正：引入有效粒子数(N_eff)计算误差，并使用加权平均能量作为绘图坐标，解决变权重粒子带来的统计偏差。"
    ...
    def _analyze_single_run(self, run: SimulationRun):
        ...
        dN_dE_theory = self._calculate_mj_theory(centers_weighted, T_keV, total_weight)
        ...
        with create_analysis_figure(run, "tail_statistics", num_plots=2,
            ...
            y_smooth = self._calculate_mj_theory(x_smooth, T_keV, total_weight)
