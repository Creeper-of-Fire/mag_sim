...
def _center_field(field: np.ndarray, target_shape: tuple) -> np.ndarray:
    """
        将一个在3D交错网格上的场分量稳健地插值到单元中心。
        这个版本可以正确处理在1个或2个维度上交错的场。
        """
    ...
    # --- 情况1：场在一个维度上交错 (例如，面中心场) ---
    if field.shape == (nx + 1, ny, nz):
        return 0.5 * (field[:-1, :, :] + field[1:, :, :])
    if field.shape == (nx, ny + 1, nz):
        return 0.5 * (field[:, :-1, :] + field[:, 1:, :])
    if field.shape == (nx, ny, nz + 1):
        return 0.5 * (field[:, :, :-1] + field[:, :, 1:])

    # --- 情况2：场在两个维度上交错 (例如，边中心场 E) ---
    # 形状 (NX, NY+1, NZ+1) -> 在 Y-Z 平面平均
    if field.shape == (nx, ny + 1, nz + 1):
        # 先在 y 方向平均，形状变为 (nx, ny, nz + 1)
        field_y_avg = 0.5 * (field[:, :-1, :] + field[:, 1:, :])
        # 再在 z 方向平均，形状变为 (nx, ny, nz)
        return 0.5 * (field_y_avg[:, :, :-1] + field_y_avg[:, :, 1:])

    # 形状 (NX+1, NY, NZ+1) -> 在 X-Z 平面平均
    if field.shape == (nx + 1, ny, nz + 1):
        # 先在 x 方向平均，形状变为 (nx, ny, nz + 1)
        field_x_avg = 0.5 * (field[:-1, :, :] + field[1:, :, :])
        # 再在 z 方向平均，形状变为 (nx, ny, nz)
        return 0.5 * (field_x_avg[:, :, :-1] + field_x_avg[:, :, 1:])

    # 形状 (NX+1, NY+1, NZ) -> 在 X-Y 平面平均
    if field.shape == (nx + 1, ny + 1, nz):
        # 先在 x 方向平均，形状变为 (nx, ny + 1, nz)
        field_x_avg = 0.5 * (field[:-1, :, :] + field[1:, :, :])
        # 再在 y 方向平均，形状变为 (nx, ny, nz)
        return 0.5 * (field_x_avg[:, :-1, :] + field_x_avg[:, 1:, :])
...
def _calculate_field_energy(fpath: str, target_shape: tuple, cell_volume: float):
    ...
    # 能量密度 (J/m^3)
    mag_energy_density_x = (Bx ** 2) / (2 * mu_0)
    mag_energy_density_y = (By ** 2) / (2 * mu_0)
    mag_energy_density_z = (Bz ** 2) / (2 * mu_0)
    mag_energy_density_total = mag_energy_density_x + mag_energy_density_y + mag_energy_density_z

    mean_mag_edens_x = np.mean(mag_energy_density_x)
    mean_mag_edens_y = np.mean(mag_energy_density_y)
    mean_mag_edens_z = np.mean(mag_energy_density_z)
    mean_mag_edens_total = np.mean(mag_energy_density_total)
    total_magnetic_energy = np.sum(mag_energy_density_total) * cell_volume

    # 电场部分
    Ex = _center_field(Ex_s, target_shape)
    Ey = _center_field(Ey_s, target_shape)
    Ez = _center_field(Ez_s, target_shape)

    elec_energy_density_x = (epsilon_0 * Ex ** 2) / 2
    elec_energy_density_y = (epsilon_0 * Ey ** 2) / 2
    elec_energy_density_z = (epsilon_0 * Ez ** 2) / 2
    elec_energy_density_total = elec_energy_density_x + elec_energy_density_y + elec_energy_density_z

    mean_elec_edens_x = np.mean(elec_energy_density_x)
    mean_elec_edens_y = np.mean(elec_energy_density_y)
    mean_elec_edens_z = np.mean(elec_energy_density_z)
    mean_elec_edens_total = np.mean(elec_energy_density_total)

    # 盒子内总能量 (J)
    total_electric_energy = np.sum(elec_energy_density_total) * cell_volume

    return (mean_mag_edens_x, mean_mag_edens_y, mean_mag_edens_z, mean_mag_edens_total, total_magnetic_energy,
            mean_elec_edens_x, mean_elec_edens_y, mean_elec_edens_z, mean_elec_edens_total, total_electric_energy)
...
def load_energy_evolution_data(dir_path: str, sim_obj: object) -> Optional[EnergyEvolutionData]:
    ...
    for step in common_steps:
        ...
        (m_ed_x, m_ed_y, m_ed_z, m_ed_tot, tot_m_E,
         e_ed_x, e_ed_y, e_ed_z, e_ed_tot, tot_e_E) = _calculate_field_energy(fpath_field, target_shape, cell_volume)
        ...
        # 存储电场数据
        elec_edens_x.append(e_ed_x)
        elec_edens_y.append(e_ed_y)
        elec_edens_z.append(e_ed_z)
        elec_edens_tot.append(e_ed_tot)
        total_elec_E.append(tot_e_E)
    ...
    return EnergyEvolutionData(
        ...
        # 电能
        mean_elec_energy_density_x=np.array(elec_edens_x),
        mean_elec_energy_density_y=np.array(elec_edens_y),
        mean_elec_energy_density_z=np.array(elec_edens_z),
        mean_elec_energy_density_total=np.array(elec_edens_tot),
        # 动能
        mean_kin_energy_density=np.array(kin_edens),
        # 总能量
        total_magnetic_energy=np.array(total_mag_E),
        total_electric_energy=np.array(total_elec_E),
...
def generate_energy_evolution_plot(runs: List['SimulationRun']):
    ...
    for i, run in enumerate(runs):
        ...
        elec_density_x_ev = data.mean_elec_energy_density_x / J_PER_EV
        elec_density_y_ev = data.mean_elec_energy_density_y / J_PER_EV
        elec_density_z_ev = data.mean_elec_energy_density_z / J_PER_EV
        ...
        # --- 绘制分量来区分不稳定性 ---
        # 韦伯不稳定性信号: 垂直磁场
        ax_density.plot(data.time, mag_density_x_ev + mag_density_y_ev, ':', color='red', lw=2, label=r'$\langle \epsilon_{B,\perp} \rangle$ (Weibel Signal)')
        # 双流不稳定性信号: 纵向电场
        ax_density.plot(data.time, elec_density_z_ev, ':', color='cyan', lw=2, label=r'$\langle \epsilon_{E,z} \rangle$ (Two-Stream Signal)')
        ...
        ax_total.plot(data.time, total_energy_ev, '-', color='orange', lw=2, alpha=0.8, label=r'$E_{K, tot} + E_{B, tot} + E_{E, tot}$')
